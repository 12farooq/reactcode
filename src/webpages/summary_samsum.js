...
class SummarySamsum extends React.Component {
  ...
  async getAnomaliesData() {
    ...
    try {
      ...
      await axios({
        ...
        params: { "t_ids": JSON.stringify(sessionStorage.getItem('summary_tids')), }
      })
      .then(
        response => {
        ...
        {
          {
            'loading': 
              <div className="loader">
                <span className="loader-text">Fetching Anomalies...</span>
                <Loader type="Oval" color="#fe7e25" loading="true" height={20} width={20} />
              </div>,
            'empty':
              <div className="loader">
                <span className="loader-text">No Anomalies in the Selected Transcript</span>
              </div>,
            'done':
...
class Filter extends React.Component {
  ...
  removeAnomaliesTranscripts = (val) => {
    console.log(val)
    let anomalies_tids = sessionStorage.getItem('anomalies_tids').split(',').map( n => parseInt(n, 10))
    sessionStorage.setItem('anomalies_tids', anomalies_tids.filter(item => item !== val))
    this.props.refreshAnomalies()
  }
            ...
            <ul className="transcripts-list">
              ...
              {this.props.transcripts.map(row => (
                !this.state.tnameSearchQuery || row.name.toLowerCase().includes(this.state.tnameSearchQuery.toLowerCase()) ?
                  <div className="transcript-names">
                  {sessionStorage.getItem('anomalies_tids').split(',').map( n => parseInt(n, 10)).indexOf(row.id) > -1 ?

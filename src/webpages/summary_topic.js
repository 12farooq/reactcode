...
class SummaryTopic extends React.Component {
  ...
  async componentDidMount() {
    sessionStorage.getItem('summary_topic_tids') === null ? sessionStorage.setItem('summary_topic_tids', '') :void 0;

    this.isComponentMounted = true;
    // if(!sessionStorage.getItem('summary_topic_tids')){
    //   sessionStorage.setItem('summary_topic_tids', []);  
    // }
  ...
  async getAnomaliesData() {
    ...
    try {
      ...
      await axios({
        ...
        params: { "t_ids": JSON.stringify(sessionStorage.getItem('summary_topic_tids')), "topic":'topic', }
      })
      .then(
        response => {
        ...
        {
          {
            'loading': 
              <div className="loader">
                <span className="loader-text">Fetching Topical Summary...</span>
                <Loader type="Oval" color="#fe7e25" loading="true" height={20} width={20} />
              </div>,
            'empty':
              <div className="loader">
                <span className="loader-text">No Topical Summary in the Selected Transcript</span>
              </div>,
            'done':
  ...
  render() {
    return (
      <div className="card-container">
        ...
        <br />
        <div className="q1">
          {/* <span className='card-items-title'> Topical Summary:- </span> */}
...
class Filter extends React.Component {
  ...
  addAnomaliesTranscripts = (val) => {
    let summary_topic_tids = []
    if(sessionStorage.getItem('summary_topic_tids')){
      summary_topic_tids = sessionStorage.getItem('summary_topic_tids').split(',')
    }
    summary_topic_tids.push(val)
    sessionStorage.setItem('summary_topic_tids', summary_topic_tids)
    this.props.refreshAnomalies()
  }

  removeAnomaliesTranscripts = (val) => {
    console.log(val)
    let summary_topic_tids = sessionStorage.getItem('summary_topic_tids').split(',').map( n => parseInt(n, 10))
    sessionStorage.setItem('summary_topic_tids', summary_topic_tids.filter(item => item !== val))
    this.props.refreshAnomalies()
  }
            ...
            <ul className="transcripts-list">
              ...
              {this.props.transcripts.map(row => (
                !this.state.tnameSearchQuery || row.name.toLowerCase().includes(this.state.tnameSearchQuery.toLowerCase()) ?
                  <div className="transcript-names">
                  {sessionStorage.getItem('summary_topic_tids').split(',').map( n => parseInt(n, 10)).indexOf(Number(row.id)) > -1 ?
                    <li className="selected-item" onClick={() => this.removeAnomaliesTranscripts(Number(row.id))}><span>✓</span> {row.name}</li>
                    :
                    <li className="unselected-item" onClick={() => this.addAnomaliesTranscripts(Number(row.id))}>{row.name}</li>
                  }
                  </div>
                  :
                  null
